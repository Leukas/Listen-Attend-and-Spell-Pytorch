...
# test_set = create_dataloader(conf['meta_variable']['data_path']+'/test.csv', 
...
idx2char = {}
with open(conf['meta_variable']['data_path']+'/idx2chap2.csv','r') as f:
    for line in f:
        if 'idx' in line:continue
        idx2char[int(line.split(',')[0])] = line[:-1].split(',')[1]

# Load pre-trained model
global_step = 0 # conf['training_parameter']['pretrained_step']
...
def restore_words(labels):
    ...
    for i in range(labels.size(0)):
        ...
        for c in range(labels.size(1)):
            # print(labels[i,c])
            # print(idx2char[labels[i,c].item()])
...
#             break
    ...
    for batch_data,batch_label in valid_set:
        ...
        # print(batch_label)
        # sentences = restore_words(batch_label)
